...
# test custom reverse tree, by addind number for ordering before and a 1 to keep 0s
...
k100 <- plotTSNE(sce, colour_by = "cluster_oligo_k100", text_by="cluster_oligo_k100", force=0 ) +
  theme( legend.position = "none") + ggtitle("Clustering k100")
k150 <- plotTSNE(sce, colour_by = "cluster_oligo_k150", text_by="cluster_oligo_k150", force=0 )+
  theme( legend.position = "none") + ggtitle("Clustering k150")
k120 <- plotTSNE(sce, colour_by = "cluster_oligo_k120", text_by="cluster_oligo_k120", force=0 ) +
  theme( legend.position = "none") + ggtitle("Clustering k120")
k10 <- plotTSNE(sce, colour_by = "cluster_oligo_k10", text_by="cluster_oligo_k10", force=0 ) +
  theme( legend.position = "none") + ggtitle("Clustering k10")
